...
-(void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    {
        //dbg msg
        os_log_debug(logHandle, "mFirewall running from %{public}@, not from within /Applications", NSBundle.mainBundle.bundlePath);
        
        //show alert
        showAlert(NSAlertStyleInformational, NSLocalizedString(@"mFirewall must run from within /Applications\r\n", @"mFirewall must run from within /Applications\r\n"), NSLocalizedString(@"...please copy it into /Applications and re-launch.", @"...please copy it into /Applications and re-launch."), @[NSLocalizedString(@"OK",@"OK")]);
...
-(IBAction)quit:(id)sender
{
    ...
    //show alert
    response = showAlert(NSAlertStyleInformational, NSLocalizedString(@"Quit mFirewall?", @"Quit mFirewall?"), NSLocalizedString(@"...this will terminate mFirewall, until the next time you log in.", @"...this will terminate mFirewall, until the next time you log in."), @[NSLocalizedString(@"Quit", @"Quit"), NSLocalizedString(@"Cancel", @"Cancel")]);
    ...
    else
    {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                // disable the network filter so mFirewall stops filtering (user's expectation on quit)
...
-(IBAction)uninstall:(id)sender
{
    ...
    //show alert
    response = showAlert(NSAlertStyleInformational, NSLocalizedString(@"Uninstall mFirewall?", @"Uninstall mFirewall?"), NSLocalizedString(@"...this will fully remove mFirewall from your Mac", @"...this will fully remove mFirewall from your Mac"), @[NSLocalizedString(@"Uninstall", @"Uninstall"), NSLocalizedString(@"Cancel", @"Cancel")]);
