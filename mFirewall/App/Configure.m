...
-(BOOL)install
{
    ...
    //remove any existing mFirewall.app
...
-(BOOL)uninstall
{
    ...
    //quit (other) mFirewalls, network monitor, and deactivate the system extension
    [self quit:YES];
...
-(void)quit:(BOOL)deactivateExtension
{
    ...
    for(NSDictionary* lulu in findProcesses(@"mFirewall"))
    {
        //pid
        NSNumber* pid = 0;
...
// unless its the non-mFirewall version
-(void)terminateNetworkMonitor
{
    //find match
    // will check if mFirewall's, then will terminate
